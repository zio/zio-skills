...
- **Repo**: the owner/repo used`);
  ...
  // Handled in a single writer session with two sequential prompts so the agent
  // retains the decision context when generating the document.
...
Do not proceed to the next step until the current one succeeds.`,
  ...
  // Phase 5: Verify Lint — only if Scala example files were touched
...
If any \`schema-examples/src/main/scala/**/*.scala\` files appear:
\`\`\`bash
cd ${projectRoot} && git add schema-examples/src/main/scala/**/*.scala && sbt fmtChanged
...
- LINT_FAILED: <error details> — if either check failed`);
    ...
    console.log('\n[Phase 5] ⏭ Lint skipped — no-docs-needed');
    phasesCompleted.push('lint');
  }
