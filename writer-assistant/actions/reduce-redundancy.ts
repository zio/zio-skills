...
export const reduceRedundancyAction = defineAction({
  name: 'reduce_redundancy',
  description:
    'Scan documentation for lexical, structural, and semantic redundancy, then iteratively fix until clean or max rounds reached.',
  input: v.object({
    outputPath: v.string(),
  ...
  run: (async ({ harness, input }: { harness: any; input: any }) => {
    return runReduceRedundancyPhase(harness, input);
  }) as (ctx: any) => any,
});
